...
function createTile(i, j, gridSize, referenceMaterial, painting_texture) {
    ...
    //note: 3d intersections when shaded as 2d are very interesting
    const geometry = new THREE.SphereBufferGeometry(tileSize * Math.random(), 50, 50);
...
function animate() {
    ...
    let s = Math.sin;
    let camTime = time * 0.1;
    sphereCam.position.set(s(camTime*0.3+100), s(camTime*0.33+5), s(camTime*0.36 + 20));
...
void main()	{
    ...
    float rowDev = (hash(vec3(quant(vUv.y, 10.), 10., 10.)).y - 0.5) * 0.015;
